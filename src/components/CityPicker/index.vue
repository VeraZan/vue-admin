...
import { watch,watchEffect, onBeforeMount, reactive } from '@vue/composition-api';
export default {
  name: "cityPicker",
  //mixins:[cityPicker],
  props: {
  ...
  setup(props, { emit }){
    ...
    // watchEffect(() =>{
    //   if(Object.keys(props.cityPickerData).length !== 0){      
    //     data.provinceValue = props.cityPickerData.provinceValue;
    //     if(props.cityPickerData.cityValue){                    
    //       new Promise(resolve => {
    //         handlerProvince(props.cityPickerData.provinceValue);
    //         resolve(props.cityPickerData.cityValue);
    //       }).then(res => {
    //         data.cityValue = res;
    //       })
    //       if(props.cityPickerData.areaValue){
    //         new Promise(resolve => {
    //           handlerCity(props.cityPickerData.cityValue);
    //           resolve(props.cityPickerData.areaValue);
    //         }).then(res => {
    //           data.areaValue = res;
    //         })                      
    //         if(props.cityPickerData.streetValue){
    //           new Promise(resolve => {
    //             handlerArea(props.cityPickerData.areaValue);
    //             resolve(props.cityPickerData.streetValue);
    //           }).then(res => {
    //             data.streetValue = res;
    //           })            
    //         }
    //       }
    //     }                       
